...
## Total shooting incident trends by murdered or survived from 2006 to 2020
...
# Victim
## Total shooting incident by victim’s race
...
## Total shooting incident by victim’s sex
```{r echo = FALSE, message=FALSE}
vic_sex_plot = 
  data_hist %>% 
  filter(year >= 2006,
...
## Total shooting incident by victim’s age group
...
## Total shooting incident by jurisdiction_code
```{r echo = FALSE, message = FALSE}
...
## Total shooting incident by hour in a day for each borough
...
education_plot = 
  ...
  ggplot(aes(x = year, y = percent, color = boro)) + scale_x_continuous(breaks = seq(20, 2020, by = 1)) +
  geom_line() +
  labs(
    title = "Education", 
    x =  "Year",
