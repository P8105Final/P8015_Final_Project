...
```{r}
regression_df = data_hist %>% filter(year >= 2010) %>% select(-c(incident_key,x_coord_cd,y_coord_cd,vic_age_group,vic_race,vic_sex,lon_lat))

data_unemployed_df = left_join(regression_df,unemployed_complete,by = c("year","month"))

combined_df = left_join(data_unemployed_df,education_df, by = c("year","boro"))
```

```{r}
write.csv(combined_df, "data/combined.csv")
```
...
total_df = combined_df %>% 
  ...
  count(boro, name = "n_incident") 

reg_df = left_join(combined_df,total_df, by = c("year","boro","month")) %>%
  mutate(perp_sex = factor(perp_sex),
  perp_race = factor(perp_race),
  perp_age_group = factor(perp_age_group))


mod = lm(n_incident ~ perp_sex + perp_race+perp_age_group, data = reg_df)
