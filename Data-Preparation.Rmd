...
```{r}
pal <- colorFactor(c("navy", "red"), domain = data_hist$statistical_murder_flag)

<<<<<<< HEAD:Data Preparation.Rmd
...
=======
>>>>>>> 7cd4eb9ec8bdbf3e50afa60461878a013bf4ddc9:Data-Preparation.Rmd

data_map = 
  data_hist %>%
  filter(boro=="Manhatten") %>% 
  mutate(
    date_paste = as.Date(paste(year,month,day,sep = "-")),
    time_paste = paste(hour,minute,sep = ":")) %>% 
  rename("lat" = "latitude",
         "long" = "longitude") %>%
  select(date_paste,time_paste, statistical_murder_flag, lat,long)
